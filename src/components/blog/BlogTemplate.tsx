...
export default function BlogTemplate({ title, category, readTime, content, tier = "Advanced" }: any) {
  return (
    <div className="bg-brand-black min-h-screen text-brand-cream font-sans pb-32">
      <nav className="p-8 border-b border-white/5 bg-black/50 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <Link href="/blog" className="inline-flex items-center text-brand-neon hover:text-white transition-colors group">
            <ArrowLeft className="mr-2 w-5 h-5 group-hover:-translate-x-1 transition-transform" /> Back to Intelligence Hub
          </Link>
          <div className="flex gap-4">
             <Share2 className="w-5 h-5 text-brand-cream/40 hover:text-brand-neon cursor-pointer" />
             <Heart className="w-5 h-5 text-brand-cream/40 hover:text-red-500 cursor-pointer" />
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-4xl mx-auto px-6 pt-20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-4 mb-8">
            ...
            <div className="flex items-center gap-2 text-xs font-mono text-brand-cream/40 uppercase">
              <Clock className="w-3 h-3" /> {readTime} Min Read
            </div>
            <div className="flex items-center gap-2 text-xs font-mono text-brand-cream/40 uppercase border-l border-white/10 pl-4">
              <Database className="w-3 h-3" /> {tier} Tier
            </div>
          </div>

          <h1 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter mb-12 leading-[1.1]">
            {title}
          </h1>

          <div className="prose prose-invert prose-lg max-w-none">
            {content.split('\n\n').map((para: string, i: number) => (
              <p key={i} className="mb-8 font-light leading-relaxed text-brand-cream/80">
                {para}
              </p>
            ))}
          </div>
          ...
          <div className="mt-20 p-8 border border-brand-neon/20 bg-brand-neon/5 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-8">
             <div>
                <h3 className="text-xl font-bold text-white uppercase mb-2">Protocol Ready?</h3>
                <p className="text-sm text-brand-cream/60">Generate a custom stack based on this research using our Hebe agent.</p>
             </div>
