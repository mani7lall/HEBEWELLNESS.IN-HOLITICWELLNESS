...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Intelligence", link: "/blog", hoverColor: "hover:text-white" }
  ];

  return (
    <>
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl z-50">
        <div className="glass-panel px-6 lg:px-8 py-4 rounded-full flex justify-between items-center transition-all duration-300">
          
          {/* Advanced 2026 Logo mark */}
          <Link href="/" className="font-sans font-black text-xl lg:text-2xl tracking-tighter uppercase text-white hover:text-brand-neon-cyan transition-colors relative group z-50">
          ...
          <div className="hidden lg:flex items-center gap-6">
            {navLinks.map((item, idx) => (
              <Link key={idx} href={item.link} className={`text-[10px] font-bold tracking-[0.2em] ${idx === 5 ? 'text-white/50' : 'text-white'} ${item.hoverColor} uppercase transition-colors`}>{item.name}</Link>
            ))}
          </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-40 bg-brand-black/95 backdrop-blur-3xl flex flex-col items-center justify-center lg:hidden">
          <div className="flex flex-col gap-8 text-center w-full px-6">
            {navLinks.map((item, idx) => (
              <Link key={idx} href={item.link} onClick={() => setIsOpen(false)} className={`text-3xl font-black tracking-widest uppercase text-white/80 ${item.hoverColor} transition-colors`}>
                {item.name}
              </Link>
            ))}
