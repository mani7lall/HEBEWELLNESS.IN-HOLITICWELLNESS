  ...
  return (
    <div className="bg-brand-black min-h-screen text-brand-cream font-sans pb-32">
      ...
      <main className="max-w-6xl mx-auto px-6">
        <h1 className="text-5xl md:text-7xl font-black text-white uppercase tracking-tighter mb-12">The <span className="text-brand-neon">Community Standards</span></h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {standards.map((std, i) => (
            <div key={i} className="bg-white/5 border border-white/10 p-10 rounded-2xl group hover:border-brand-neon transition-colors">
              ...
              <h2 className="text-3xl font-bold text-white uppercase mb-4 group-hover:text-brand-neon transition-colors">{std.title}</h2>
              <p className="text-brand-cream/70 font-light leading-relaxed">{std.description}</p>
            </div>
          ))}
        </div>
      </main>
