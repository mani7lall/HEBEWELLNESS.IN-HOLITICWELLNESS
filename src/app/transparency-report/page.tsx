  ...
  return (
    <div className="bg-brand-black min-h-screen text-brand-cream font-sans pb-32">
      ...
      <main className="max-w-6xl mx-auto px-6">
        <h1 className="text-5xl md:text-7xl font-black text-white uppercase tracking-tighter mb-12">The <span className="text-brand-neon">Transparency Report</span></h1>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {reports.map((report, i) => (
            <div key={i} className="bg-white/5 border border-white/10 p-8 rounded-2xl group hover:border-brand-neon transition-colors">
              ...
              <h2 className="text-2xl font-bold text-white uppercase mb-2 group-hover:text-brand-neon transition-colors">{report.title}</h2>
              <p className="text-brand-neon font-mono text-xs uppercase mb-6 tracking-widest">{report.status}</p>
              <p className="text-brand-cream/70 font-light leading-relaxed text-sm mb-6">{report.details}</p>
              <button className="w-full py-2 border border-white/20 text-white font-bold uppercase text-[10px] tracking-widest hover:bg-white hover:text-black transition-all rounded-sm">Download COA Data</button>
            </div>
          ))}
        </div>
      </main>
