...
export default function CategoryPage() {
  ...
  if (!data) return <div className="p-20 text-center">Category Loading...</div>;

  const Icon = data.icon;

  return (
    <div className="bg-brand-black min-h-screen text-brand-cream pt-32 pb-20 px-6 lg:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-center mb-32">
          ...
          >
            ...
            <p className="text-xl md:text-2xl text-brand-cream/60 font-light mb-10 leading-relaxed">
              {data.tagline}
            </p>
          ...
          <motion.div 
            ...
            className="lg:w-1/2 relative bg-white/5 p-1 rounded-[3rem] border border-white/10"
          >
            <div className="bg-brand-black rounded-[2.8rem] p-12 overflow-hidden relative">
              <Icon className={`w-32 h-32 ${data.color} opacity-20 absolute -top-10 -right-10`} />
              <h3 className="text-2xl font-black text-white mb-6">Domain Summary</h3>
              <p className="text-brand-cream/60 leading-relaxed mb-10 font-light">
                {data.description}
              </p>
              <div className="grid grid-cols-2 gap-4">
                 <div className="p-6 bg-white/5 rounded-2xl border border-white/10 text-center">
                    <div className="text-3xl font-black text-white mb-1">94%</div>
                    <div className="text-[10px] uppercase font-bold text-brand-cream/40">Success Rate</div>
                 </div>
                 <div className="p-6 bg-white/5 rounded-2xl border border-white/10 text-center">
                    <div className="text-3xl font-black text-white mb-1">12+</div>
                    <div className="text-[10px] uppercase font-bold text-brand-cream/40">Papers Published</div>
                 </div>
              </div>
            </div>
          </motion.div>
        ...
        <section className="mb-32">
          <div className="bg-gradient-to-br from-brand-dark to-brand-black p-12 lg:p-20 rounded-[4rem] border border-white/10 shadow-2xl relative overflow-hidden">
             ...
             <div className="relative z-10 flex flex-col lg:flex-row gap-16 items-center">
                <div className="lg:w-1/3 text-center lg:text-left">
                   ...
                   <h2 className="text-4xl font-black text-white uppercase tracking-tighter mb-4">The {data.gummy.name}</h2>
                   <div className="text-2xl font-black text-brand-neon mb-6">{data.gummy.price}</div>
                   <button className="w-full bg-white text-brand-black font-black uppercase tracking-widest py-4 rounded-xl hover:bg-brand-neon transition-all shadow-glow">
                      Coming Soon - Join Waitlist
                   </button>
                </div>
                <div className="lg:w-2/3 grid md:grid-cols-2 gap-8">
                   <div className="bg-white/5 p-8 rounded-3xl border border-white/10">
                      <h4 className="text-xs uppercase font-black tracking-widest text-brand-cream/40 mb-6 flex items-center gap-2">
                        <CheckCircle2 className={`w-4 h-4 ${data.color}`} /> Clinical Composition
                      </h4>
                      <ul className="space-y-4">
                        {data.gummy.ingredients.map((ing, i) => (
                           <li key={i} className="text-lg font-bold text-white flex justify-between">
                   ...
                   <div className="bg-white/5 p-8 rounded-3xl border border-white/10">
                      ...
                      <p className="text-sm font-light text-brand-cream/60">
