    ...
    <div className="min-h-screen bg-brand-black text-brand-cream py-32 px-6">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-xl font-light text-brand-cream/60">Architect your AM/PM biological support system based on outcome and budget.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-12">
          {Object.keys(stacks).map((s) => (
             <button 
               key={s} 
               onClick={() => setGoal(s)}
               className={`p-10 rounded-[2.5rem] border transition-all text-center group ${goal === s ? 'bg-brand-neon-cyan text-brand-black border-brand-neon-cyan shadow-glow' : 'bg-white/5 border-white/10 hover:border-brand-neon-cyan/50'}`}
             >
          ...
          {goal ? (
            ...
            >
              <div className="bg-white/5 border border-white/10 p-12 rounded-[3rem]">
                 ...
                 <ul className="space-y-6">
                    {stacks[goal as keyof typeof stacks].AM.map((item, i) => (
                       <li key={i} className="flex items-center justify-between p-4 bg-black/40 border border-white/5 rounded-2xl">
              ...
              <div className="md:col-span-2 bg-brand-neon-cyan/10 border border-brand-neon-cyan/40 p-12 rounded-[3rem] text-center">
                 <h3 className="text-2xl font-black text-white mb-4">Total Estimated Monthly Cost: <span className="text-brand-neon-cyan">${stacks[goal as keyof typeof stacks].cost}/mo</span></h3>
                 <p className="text-brand-cream/60 mb-10 font-light italic">"These dosages are peer-reviewed for a 75kg baseline subject."</p>
                 
                 <div className="max-w-2xl mx-auto p-10 bg-black/40 rounded-3xl border border-white/10">
                    <Mail className="w-12 h-12 text-brand-neon-cyan mx-auto mb-6"/>
                    <h4 className="text-2xl font-black text-white mb-2 uppercase">Unlock Sourcing Guide</h4>
                    <p className="text-sm text-brand-cream/50 mb-8 font-light">Enter your email to receive our vetted whitelist of 3rd party batch-tested suppliers for this exact stack.</p>
                    <form className="flex flex-col sm:flex-row gap-4">
                       <input type="email" placeholder="Sourcing Dispatch Email" className="flex-1 bg-black border border-white/10 rounded-xl px-6 py-4 text-white outline-none focus:border-brand-neon-cyan" />
            ...
            <div className="h-96 flex items-center justify-center border-2 border-dashed border-white/10 rounded-[3rem] text-brand-cream/20 font-black uppercase tracking-widest text-2xl">
              Selection Required
            </div>
          )}
        </AnimatePresence>
        ...
        <div className="mt-16 text-center">
           <Link href="/" className="text-brand-cream/40 hover:text-brand-neon-cyan transition-colors text-xs font-bold uppercase tracking-[0.2em] flex justify-center items-center gap-3">
