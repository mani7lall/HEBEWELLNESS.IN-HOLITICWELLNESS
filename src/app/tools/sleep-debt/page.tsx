  ...
  return (
    <div className="bg-white dark:bg-white dark:bg-brand-black min-h-screen text-slate-900 dark:text-white font-sans pt-32 pb-24">
      <div className="max-w-3xl mx-auto px-6">
        <Link href="/tools" className="inline-flex items-center text-blue-400 hover:text-slate-900 dark:text-white font-mono text-sm uppercase tracking-widest mb-12">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-4">Sleep Debt <span className="text-blue-400">Calculator</span></h1>
          <p className="text-slate-900 dark:text-white/60 max-w-xl mx-auto">Calculate your physiological sleep debt and generate a 2026 recovery protocol to restore your autonomic nervous system.</p>
        </div>

        <div className="bg-[#121c17] border border-black/10 dark:border-black/10 dark:border-white/10 p-8 rounded-3xl">
          {!calculated ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
              <div>
                <label className="block text-sm font-mono text-blue-400 mb-2 uppercase">Ideal Nightly Sleep (Hours)</label>
                <p className="text-xs text-slate-900 dark:text-white/40 mb-4">How much sleep do you need to feel 100% rested?</p>
                ...
                <div className="text-2xl font-bold text-slate-900 dark:text-white mt-2">{idealHours} hrs</div>
              </div>
              
              <div>
                <label className="block text-sm font-mono text-blue-400 mb-2 uppercase">Actual Average Sleep (Hours)</label>
                <p className="text-xs text-slate-900 dark:text-white/40 mb-4">Over the past week, how much did you actually get?</p>
                ...
                <div className="text-2xl font-bold text-slate-900 dark:text-white mt-2">{actualHours} hrs</div>
              </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
              <h2 className="text-xl font-mono text-slate-900 dark:text-white/60 uppercase mb-2">Total Accumulated Sleep Debt</h2>
              <div className="`text-7xl font-black mb-6 ${severe ? 'text-red-400' : 'text-blue-400'}`">
                {debt > 0 ? `${debt} hrs` : 'None!'}
              </div>
              
              {debt > 0 ? (
                <div className="bg-gray-50 dark:bg-gray-50 dark:bg-black/50 p-6 rounded-xl border border-black/5 dark:border-black/5 dark:border-white/5 text-left mb-8">
                  <h3 className="font-bold text-slate-900 dark:text-white mb-2 flex items-center"><Activity className="w-4 h-4 mr-2 text-blue-400"/> Clinical Impact</h3>
                  <p className="text-sm text-slate-900 dark:text-white/70 mb-4">
                    {severe 
              ...
              <button onClick={() => setCalculated(false)} className="px-6 py-3 border border-white/20 text-slate-900 dark:text-white font-bold uppercase tracking-widest rounded-lg hover:bg-white/5 transition-colors">
                Recalculate
              </button>
            </motion.div>
          )}
