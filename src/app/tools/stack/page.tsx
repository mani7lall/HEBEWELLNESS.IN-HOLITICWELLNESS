  ...
  return (
    <div className="min-h-screen pt-32 pb-24 px-6 max-w-7xl mx-auto flex flex-col items-center">
      ...
      <div className="text-center mb-16 relative z-10 w-full">
        ...
        <h1 className="text-5xl md:text-8xl font-black tracking-tight text-white mb-6 uppercase block max-w-5xl mx-auto leading-none">
          Construct Your <br/><span className="text-gradient-neon">Longevity Stack.</span>
        </h1>
        <p className="text-xl text-brand-cream/60 max-w-2xl mx-auto font-light mb-12">
      ...
      <div className="flex flex-col lg:flex-row gap-8 w-full relative z-10">
        ...
        <div className="lg:w-2/3 space-y-4">
          <h3 className="text-white text-sm tracking-widest uppercase font-bold mb-6 flex items-center gap-3"><Activity className="text-brand-neon"/> Available Compounds</h3>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {compounds.map(c => (
              <div 
                ...
                className={`glass border rounded-2xl p-6 cursor-pointer transition-all duration-300 magnetic-button relative overflow-hidden group ${selectedTools.includes(c.id) ? 'border-brand-neon shadow-glow bg-brand-neon/5' : 'border-white/10 hover:border-white/30 hover:bg-white/5'}`}
              >
                <div className="flex justify-between items-start mb-4">
                  <div className={`p-3 rounded-xl ${selectedTools.includes(c.id) ? 'bg-brand-neon text-brand-black' : 'bg-black/50 text-white'}`}>
                    {c.icon}
                  </div>
                ...
                <p className="text-sm text-brand-cream/50 leading-relaxed font-light">{c.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:w-1/3">
          <div className="sticky top-32 glass-panel p-8 rounded-3xl border border-white/10 mt-14 lg:mt-0">
            <h3 className="text-white text-sm tracking-widest uppercase font-bold mb-8 border-b border-white/10 pb-4">Architectural Readout</h3>
            
            <div className="mb-12">
              <div className="flex justify-between items-end mb-4">
                <span className="text-brand-cream/60 text-sm">Target Efficacy</span>
                <span className="text-3xl font-black text-brand-neon glow-text">{calculateEfficacy()}%</span>
              </div>
              <div className="w-full bg-black/50 rounded-full h-2 relative overflow-hidden">
                <motion.div 
                  initial={{ width: 0 }}
            ...
            <div className="space-y-4 mb-12">
                ...
                {selectedTools.map(id => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <button onClick={(e) => { e.stopPropagation(); toggleCompound(id); }} className="text-white/40 hover:text-brand-neon">
                        <X className="w-4 h-4" />
                      </button>
                    </motion.div>
                  );
              ...
              {selectedTools.length === 0 && (
                <div className="text-brand-cream/30 text-sm italic py-4 text-center border border-dashed border-white/10 rounded-lg">
            ...
            <button 
              ...
              className={`w-full py-5 rounded-full text-xs tracking-widest uppercase font-black transition-all flex items-center justify-center gap-2 ${selectedTools.length > 0 ? 'bg-brand-neon text-brand-black hover:bg-white shadow-glow-strong' : 'bg-white/5 text-white/30 cursor-not-allowed border border-white/5'}`}
            >
