  ...
  return (
    <div className="bg-white dark:bg-white dark:bg-brand-black min-h-screen text-slate-900 dark:text-white font-sans pb-32">
      ...
      <main className="max-w-6xl mx-auto px-6">
        <h1 className="text-5xl md:text-7xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-12">The <span className="text-brand-neon">Future Workforce</span></h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {jobs.map((job, i) => (
            <div key={i} className="bg-white/5 border border-black/10 dark:border-black/10 dark:border-white/10 p-10 rounded-2xl group hover:border-brand-neon transition-colors">
              <h2 className="text-3xl font-bold text-slate-900 dark:text-white uppercase mb-2 group-hover:text-brand-neon transition-colors">{job.title}</h2>
              <p className="text-brand-neon font-mono text-xs uppercase mb-6 tracking-widest">{job.location}</p>
              <p className="text-slate-900 dark:text-white/70 font-light leading-relaxed text-sm mb-6">{job.description}</p>
