...
export default function AboutPage() {
  return (
    <div className="bg-gradient-animated min-h-screen text-slate-900 dark:text-white font-sans pt-32 pb-24 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
        ...
        >
          ...
          <h1 className="text-5xl md:text-7xl font-bold text-slate-900 dark:text-white mb-8 tracking-tight drop-shadow-lg">Where Vedic Tradition Meets Swiss Biotech.</h1>
          <p className="text-xl text-slate-900 dark:text-white/60 leading-relaxed font-light">
        ...
        <div className="grid md:grid-cols-2 gap-16 mb-32 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-5xl font-bold text-slate-900 dark:text-white mb-6">The High-Altitude Advantage</h2>
            <p className="text-slate-900 dark:text-white/60 text-lg leading-relaxed mb-6">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative aspect-square">
            ...
            <div className="glass-panel w-full h-full rounded-[3rem] p-8 border border-black/10 dark:border-black/10 dark:border-white/10 relative overflow-hidden backdrop-blur-3xl flex items-center justify-center">
              <div className="grid grid-cols-2 gap-6 w-full h-full text-center">
                <div className="bg-gray-50 dark:bg-gray-50 dark:bg-black/40 rounded-2xl flex flex-col items-center justify-center border border-black/5 dark:border-black/5 dark:border-white/5 p-4 hover:border-brand-neon/50 transition-colors group">
                  <Sprout className="w-10 h-10 text-brand-neon mb-4 group-hover:scale-110 transition-transform" />
                  <span className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-widest">Pristine Soil</span>
                </div>
                <div className="bg-gray-50 dark:bg-gray-50 dark:bg-black/40 rounded-2xl flex flex-col items-center justify-center border border-black/5 dark:border-black/5 dark:border-white/5 p-4 hover:border-brand-neon/50 transition-colors group">
                  <Sparkles className="w-10 h-10 text-brand-neon mb-4 group-hover:scale-110 transition-transform" />
                  <span className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-widest">High Terpenes</span>
                </div>
                <div className="bg-gray-50 dark:bg-gray-50 dark:bg-black/40 rounded-2xl flex flex-col items-center justify-center border border-black/5 dark:border-black/5 dark:border-white/5 p-4 hover:border-brand-neon/50 transition-colors group">
                  <Network className="w-10 h-10 text-brand-neon mb-4 group-hover:scale-110 transition-transform" />
                  <span className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-widest">CO2 Extracted</span>
                </div>
                <div className="bg-gray-50 dark:bg-gray-50 dark:bg-black/40 rounded-2xl flex flex-col items-center justify-center border border-black/5 dark:border-black/5 dark:border-white/5 p-4 hover:border-brand-neon/50 transition-colors group">
                  <Shield className="w-10 h-10 text-brand-neon mb-4 group-hover:scale-110 transition-transform" />
                  <span className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-widest">Triple Tested</span>
                </div>
              </div>
            </div>
          </motion.div>
