  ...
  return (
    <div className="min-h-screen pt-40 pb-32 px-6 max-w-7xl mx-auto relative z-10 selection:bg-brand-neon">
      ...
      <div className="mb-20">
        ...
        <p className="text-xl text-brand-cream/60 max-w-2xl font-light border-l border-brand-neon pl-6">
      ...
      <div className="flex flex-wrap gap-4 mb-12">
        {categories.map(cat => (
          <button 
            ...
            className={`px-6 py-3 rounded-full text-xs font-bold uppercase tracking-widest transition-all duration-300 magnetic-button ${filter === cat ? 'bg-brand-neon text-brand-black shadow-glow' : 'glass hover:bg-white/10 text-white'}`}
          >
            {cat}
          </button>
        ))}
        ...
        <AnimatePresence>
          {filtered.map((p, i) => (
            <motion.div 
              ...
              key={p.id} 
              className="glass-panel p-8 md:p-12 rounded-[2rem] border border-white/10 hover:border-brand-neon/40 transition-colors group relative overflow-hidden"
            >
              ...
              <div className="flex flex-col md:flex-row gap-8 md:gap-16 relative z-10">
                <div className="md:w-1/3">
                  ...
                  <h3 className="text-3xl font-black text-white mb-6 leading-tight group-hover:text-brand-neon transition-colors">{p.name}</h3>
                  <div className="mb-6">
                    <span className="text-[10px] text-white/40 uppercase tracking-widest block mb-2">Primary Targets</span>
                    <div className="flex flex-wrap gap-2">
                      {p.targets.map((t, j) => (
                        <span key={j} className="text-xs bg-black/50 border border-white/10 px-3 py-1 rounded-full text-brand-cream/70">{t}</span>
                      ))}
                    </div>
                  </div>
                </div>

                <div className="md:w-2/3 flex flex-col justify-between border-t md:border-t-0 md:border-l border-white/10 pt-8 md:pt-0 md:pl-16">
                  <div>
                    <h4 className="text-sm text-brand-neon uppercase tracking-widest font-bold mb-4">Mechanism Synthesis</h4>
                    <p className="text-lg text-brand-cream/70 leading-relaxed font-light mb-8">{p.summary}</p>
                    
                    <div className="grid sm:grid-cols-2 gap-6 mb-8 bg-black/40 p-6 rounded-2xl border border-white/5">
                      <div>
                        <span className="text-[10px] text-white/40 uppercase tracking-widest block mb-1">Clinical Dose</span>
                        <span className="text-sm font-bold text-white font-mono">{p.clinicalDose}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-white/40 uppercase tracking-widest block mb-1">Administration Timing</span>
                        <span className="text-sm font-bold text-white font-mono">{p.timing}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-[10px] text-white/40 uppercase tracking-widest block mb-2">Synergy Threat Level</span>
                    ...
                    <Link href="/tools/stack" className="hidden sm:flex items-center gap-2 text-xs font-black uppercase tracking-widest text-white hover:text-brand-neon transition-colors group/btn">
